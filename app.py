...
# 화면에 보일 순서
TOP_ORDER = ["국토종주코스", "제주환상자전거길", "그랜드슬램코스"]
...
def load_routes(src: str | Path | bytes) -> pd.DataFrame:
    ...
    # CSV category 무시하고 새 상위 카테고리 적용
    df["category"] = df["route"].map(GROUP_MAP).fillna("그랜드슬램코스")
    return df
...
# ───────────────────────────────────────────────────────────────────────────────
    ...
    st.info("✅ 좌표가 비어도 기본좌표/leg_km로 계산합니다. centers.csv에 더 정확한 좌표·구간거리(leg_km)를 넣으면 그 값을 우선 사용합니다.")
