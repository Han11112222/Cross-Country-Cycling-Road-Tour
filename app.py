...
TOP_ORDER = ["그랜드슬램코스", "국토종주코스", "4대강코스", "기타코스"]  # 노출 순서

# ───────────────────────────────────────────────────────────────────────────────
# 2) 인증센터 좌표 기본값(낙동강 11개) — centers.csv에 좌표가 없으면 자동 보정
#    ※ ±1~2km 근사치. 필요시 파일에서 더 정확히 덮어쓰세요.
# ───────────────────────────────────────────────────────────────────────────────
...
def load_routes(src: str | Path | bytes) -> pd.DataFrame:
    ...
    # CSV의 category 무시하고 상위 그룹으로 덮어쓰기
...
# ───────────────────────────────────────────────────────────────────────────────
    ...
    if "seq" in dfc.columns:
        ...
        dfc = dfc.sort_values(["route","center"]).reset_index(drop=True)
    dfc["완료"] = dfc["id"].isin(st.session_state.done_center_ids)

    with st.expander("인증센터 체크(간단 편집)", expanded=True):
        edited = st.data_editor(
            dfc[["category","route","seq","center","address","완료"] if "seq" in dfc.columns
                else ["category","route","center","address","완료"]],
            use_container_width=True, hide_index=True, key="editor_centers",
        )

    # 반영
    key_series = dfc["center"].astype(str) + "|" + dfc["route"].astype(str)
    id_map = dict(zip(key_series, dfc["id"]))
    new_done = set()
    for _, row in edited.iterrows():
    ...
    # 세그먼트(센터 i → i+1) 구성: (1) leg_km > (2) 좌표 하버사인 > (3) 0
    seg_rows = []
    for route, g in dfc.groupby("route"):
        ...
        for i in range(len(recs) - 1):
            a, b = recs[i], recs[i+1]
            # 1순위: leg_km
            if "leg_km" in a and not pd.isna(a.get("leg_km")):
                dist = float(a["leg_km"])
            else:
    ...
    if not geo.empty:
        ...
        geo_ren = geo.rename(columns={"lat":"latitude","lng":"longitude"})  # ✅ rename 결과를 변수에 담아 사용
        layers.append(pdk.Layer(
            "ScatterplotLayer",
            geo_ren,
        ...
        vlat, vlng = float(geo_ren["latitude"].mean()), float(geo_ren["longitude"].mean())  # ✅ 여기서도 geo_ren 사용
    else:
