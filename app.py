# app.py — 다중 노선 확실 렌더(리스트 데이터), 뷰포인트 보정, 인증센터 latitude KeyError 차단
...
def normalize_route_name(n:str)->str:
    n=str(n).strip()
    return "제주환상" if n=="제주환상자전거길" else n
...
def view_from(paths:list[list[list[float]]], centers:pd.DataFrame|None, base_zoom:float)->tuple[float,float,float]:
    pts=[]
    for p in paths or []:
        for xy in (p or []):
    ...
    if centers is not None and not centers.empty:
        pts += centers[["lat","lng"]].dropna().astype(float).values.tolist()
    if pts:
...
@st.cache_data
def load_centers(src, auto_geo:bool)->pd.DataFrame|None:
    if src is None: return None
    df=pd.read_csv(src)
    need={"route","center","address","lat","lng","id","seq"}; miss=need-set(df.columns)
    ...
    if auto_geo:
        ...
        for i,row in needs.iterrows():
            ...
            if lat is not None and lng is not None:
                df.at[i,"lat"]=lat; df.at[i,"lng"]=lng; time.sleep(1.0)
    df["big"]=df["route"].map(ROUTE_TO_BIG).fillna("기타")
    df["big"]=pd.Categorical(df["big"],categories=TOP_ORDER,ordered=True)
    return df
...
auto_geo=st.sidebar.toggle("주소 → 좌표 자동보정",value=True)
...
if use_repo=="Repo 내 파일":
    ...
    if not routes_csv.exists(): st.error("Repo에 data/routes.csv 없음"); st.stop()
...
if tab=="🚴 구간(거리) 추적":
    ...
    big,picked=big_and_routes_selector(all_route_names+ALL_DEFINED_ROUTES,"seg",use_defined=True)

    # 경로 계산(리스트 기반으로 직접 그리기)
    path_rows=[]            # PathLayer 데이터(list of dict)
    paths_for_view=[]       # 뷰포인트 계산용
    display_rows=[]         # 요약 테이블용
    ...
    # routes.path 우선, 없으면 센터, 최후 폴백
    routes_with_path=routes.copy()
    routes_with_path["__path"]=None
    if "path" in routes_with_path.columns:
        m=routes_with_path["path"].notna()
        routes_with_path.loc[m,"__path"]=routes_with_path.loc[m,"path"].map(parse_path)

    for rname in picked:
        sub=routes_with_path[routes_with_path["route"]==rname]
        path=None; km_routes=float(sub["distance_km"].fillna(0).sum()) if not sub.empty else 0.0
        if not sub.empty and sub["__path"].notna().any():
            path=sub["__path"].dropna().iloc[0]
        if path is None:
            p2,k2=centers_polyline_and_km(rname); path=p2
            if km_routes==0 and not np.isnan(k2): km_routes=k2
        if path is None:
            path=FALLBACK_PATHS.get(rname)
        if path:
            path_rows.append({"route":rname,"path":path,"__color":[28,200,138]})
            paths_for_view.append(path)
        display_km=km_routes if km_routes>0 else float(OFFICIAL_TOTALS.get(rname,0.0))
        display_rows.append({"route":rname,"표시거리(km)":display_km})
    ...
    if total_km==0: total_km=float(pd.DataFrame(display_rows)["표시거리(km)"].fillna(0).sum())
    ...
    # 지도 — PathLayer는 리스트(dict list)로 직접 전달
    layers=[]
    if path_rows:
...
# ── 인증센터
    ...
    _,picked=big_and_routes_selector(source_routes,"cent",use_defined=True)
    ...
    for r,g in dfc.groupby("route"):
        ...
        for i in range(len(rec)-1):
            ...
            seg.append({"route":r,"start_center":a["center"],"end_center":b["center"],
    ...
    # KPI
    if not seg_df.empty:
        total=float(seg_df["distance_km"].sum())
        done=float(seg_df.loc[seg_df["done"],"distance_km"].sum())
    else:
        total=done=0.0
